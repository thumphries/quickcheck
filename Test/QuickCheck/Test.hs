...
quickCheckWithResult a p =
  (if chatty a then withStdioTerminal else withNullTerminal) $ quickCheckWithTerminal a p

-- | Tests a property, using test arguments, produces a test result,
-- and prints the results to some 'Terminal'.
quickCheckWithTerminal :: Testable prop => Args -> prop -> Terminal -> IO Result
quickCheckWithTerminal a p tm = do
